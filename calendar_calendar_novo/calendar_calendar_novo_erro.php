...
$old_error_handler = set_error_handler("calendar_calendar_novo_scriptcase_error_handler");

function calendar_calendar_novo_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['calendar_calendar_novo']['contr_erro']) && $_SESSION['scriptcase']['calendar_calendar_novo']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_calendar_calendar_novo->contr_calendar_calendar_novo->Apl_com_erro = true;
            if ($inicial_calendar_calendar_novo->contr_calendar_calendar_novo->NM_ajax_flag)
            {
                $inicial_calendar_calendar_novo->contr_calendar_calendar_novo->NM_ajax_info['errList']['geral_calendar_calendar_novo'][] = $msg;
                return;
            }
            calendar_calendar_novo_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function calendar_calendar_novo_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['calendar_calendar_novo'])  && '' != $_SESSION['scriptcase']['error_icon']['calendar_calendar_novo'])  ? $_SESSION['scriptcase']['error_icon']['calendar_calendar_novo']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['calendar_calendar_novo']) && '' != $_SESSION['scriptcase']['error_close']['calendar_calendar_novo']) ? $_SESSION['scriptcase']['error_close']['calendar_calendar_novo'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['calendar_calendar_novo']) && '' != $_SESSION['scriptcase']['error_span_title']['calendar_calendar_novo']) ? $_SESSION['scriptcase']['error_span_title']['calendar_calendar_novo'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['calendar_calendar_novo']) && '' != $_SESSION['scriptcase']['error_icon_title']['calendar_calendar_novo']) ? $_SESSION['scriptcase']['error_icon_title']['calendar_calendar_novo'] : '';
?>
